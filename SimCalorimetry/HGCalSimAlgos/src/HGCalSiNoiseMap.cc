...
HGCalSiNoiseMap::HGCalSiNoiseMap() : 
  ...
  ignoreNoise_(false) 
{
  // Three sets of parameters, gain by gain
...
HGCalSiNoiseMap::SiCellOpCharacteristics HGCalSiNoiseMap::getSiCellOpCharacteristics(const HGCSiliconDetId &cellId,
  ...
  double enc_serial(encsParam_[gain][0] 
		    + encsParam_[gain][1] * cellCap 
		    + encsParam_[gain][2] * pow(cellCap, 2));
  double enc_parallel(encpScale_ * sqrt(siop.ileak));
  // set final noise value
  siop.noise = hypot(enc_parallel, enc_serial* encCommonNoiseSub_ )  * qe2fc_;
